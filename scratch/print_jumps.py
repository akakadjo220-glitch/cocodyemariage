...
# Print jumps in token lines
prev_line = 0
for idx, (tok, val, line) in enumerate(tokens):
    if prev_line > 0 and line - prev_line > 5:
        print(f"Jump at token {idx}: Line {prev_line} -> Line {line} (token={tok}, val={val})")
    prev_line = line
